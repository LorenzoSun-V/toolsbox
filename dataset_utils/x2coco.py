...
Date: 2023-11-08 07:43:26
LastEditors: 孙家辉 sunjiahui@boton-tech.com
LastEditTime: 2024-01-12 01:31:26
Description: 
'''
import argparse
import glob
...
def voc_get_coco_annotation(obj, label2id):
    ...
    # change this to allow transferring negtive samples
    ...
    assert xmax > xmin and ymax > ymin, f"Box size error. xmax={xmax},xmin={xmin},ymax={ymax},ymin={ymin}"
    o_width = xmax - xmin
    o_height = ymax - ymin
    anno = {
...
def voc_xmls_to_cocojson(annotation_paths, label2id, output_dir, output_file):
    ...
    for a_path in tqdm(annotation_paths):
        # Read annotation xml
        with open(a_path, encoding="utf8", errors='ignore') as f:
            ann_tree = ET.parse(f)
