...
def create_and_add_plugin_node(graph, topK, keepTopK, numcls):
    ...
    mns_node = gs.Node(
        op="BatchedNMS_TRT",
        attrs=create_attrs(topK, keepTopK, numcls),
...
def create_attrs(topK, keepTopK, numcls):
...
def main():
    ...
    parser.add_argument("-t", "--topK", type=int, help="number of bounding boxes for nms", default=1000)
    parser.add_argument("-k", "--keepTopK", type=int, help="bounding boxes to be kept per image", default=20)
    parser.add_argument("-n", "--numcls", type=int, help="number class", default=1)
    args, _ = parser.parse_known_args()

    graph = gs.import_onnx(onnx.load(args.model))
    graph = create_and_add_plugin_node(graph, args.topK, args.keepTopK, args.numcls)
