...
def add_postprocess(input_name, numClasses, output_name):
    ...
    batch_size, _, num_array  = origin_output.outputs[0].shape # yolov8 shape: [batch_size, 4+cls, num_array]
    print(f"num_array: {num_array}")

    output_t = gs.Variable(name="output_t",shape=(batch_size,num_array,8),dtype=np.float32)
    ...
    ends_conf = gs.Constant("ends_conf",values=np.array([batch_size,num_array,numClasses+4],dtype=np.int64))
    ...
    # #----处理prob
    #scores = gs.Variable(name="scores",shape=(1,8400,numClasses),dtype=np.float32)
